...
client.on(Events.InteractionCreate, async interaction => {
        ...
        try {
            ...
            for (const s of stocks) {
                try {
                    ...
                    portfolio.push({ symbol: s.symbol, profit: 'N/A', news: 'N/A' });
                }
            }
            ...
            await interaction.editReply("❌ เกิดข้อผิดพลาดในการวิเคราะห์พอร์ต กรุณาลองใหม่");
        }
    ...
    // ========================
        ...
        try {
            ...
            await interaction.editReply("❌ ไม่พบข้อมูลหุ้น"); 
        }
    ...
    // ========================
    // HANDLER: analyze-diversification  ← แก้ไขหลัก
    // ========================
        ...
        try {
            ...
            // ดึงข้อมูลทุกตัวพร้อมกัน (Parallel) แทน loop ซีเรียล
            ...
            for (const result of results) {
                if (result.status === 'fulfilled') {
                    ...
                    console.error('Error fetching stock data:', result.reason?.message);
                }
            }

            if (totalValue === 0) return await interaction.editReply("❌ ไม่สามารถดึงข้อมูลราคาหรือกลุ่มอุตสาหกรรมได้ กรุณาลองใหม่");
            ...
            console.error('analyze-diversification error:', e);
            await interaction.editReply("❌ เกิดข้อผิดพลาดในการวิเคราะห์ กรุณาลองใหม่");
        }
    ...
    // ========================
        ...
        try {
            ...
            await interaction.editReply("❌ เกิดข้อผิดพลาดในการเพิ่มหุ้น");
        }

    // ========================
    // HANDLER: remove-stock
    // ========================
        ...
        try {
            ...
            try {
                ...
                await Transaction.create({ 
                    ...
                    symbol: s, 
                    type: 'SELL', 
                    amount: stockToRemove.amount,
                    price: quote.price 
                });
            } catch (e) {
                console.error("Could not record SELL transaction:", e.message);
            }
            ...
            await interaction.editReply("❌ เกิดข้อผิดพลาดในการลบหุ้น");
        }
    ...
    // ========================
        ...
        try {
            ...
            if (!stock) return await interaction.editReply(`❌ ไม่พบหุ้น **${s}** ในพอร์ตของคุณ`);
            ...
            await interaction.editReply("❌ เกิดข้อผิดพลาดในการอัปเดต");
        }
    ...
    // ========================
        ...
        try {
            ...
            await interaction.editReply("❌ เกิดข้อผิดพลาดในการดึงประวัติ");
        }
    }
});
